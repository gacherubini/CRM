...
from app.models_db import ClienteOperacionalProjecao, _agora


def apply_payload(db: Session, cliente_id: str, payload: dict[str, Any]) -> list[str]:
    """Aplica envelopes operacionais de forma monotônica e idempotente.

    Regras por aggregate (iguais a chatbot/estoque/portal apply_snapshot):
    ...
    for envelope in payload.get("operational") or []:
        ...
        if not aggregate:
            continue
        reasons.append(_apply_envelope(db, cliente_id, envelope, str(aggregate)))
    return reasons


def allows_processing(
    db: Session, cliente_id: str, module: str | None = None
) -> bool:
    """Gate mínimo: Loja ativa e, se pedido, módulo contratado e ativo.

    Fail-closed quando a projeção da loja não existe.
    """
    loja = db.get(ClienteOperacionalProjecao, (cliente_id, "loja"))
    if loja is None or loja.state != "ativa":
        return False
    if module is None:
        return True
    assigned = db.get(ClienteOperacionalProjecao, (cliente_id, module))
...
def _apply_envelope(
    ...
    existing = db.get(ClienteOperacionalProjecao, (cliente_id, aggregate))
