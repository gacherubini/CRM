...
1. **Google OAuth precisa de secrets GCP**  
   Código de OAuth, ports HTTP, outbox e a **UI de operação** (conectar, escolher conta, vincular conversões e sincronizar métricas) estão implementados pela Parte B de `docs/superpowers/specs/2026-07-29-telas-canais-wa-google-design.md`. **Não há** projeto GCP/consent/developer token configurados no lab por este repositório. Sem o conjunto completo de `GOOGLE_ADS_*` secrets, a UI mostra “Google não configurado neste ambiente” e não oferece um botão que cairia no adapter fake.
