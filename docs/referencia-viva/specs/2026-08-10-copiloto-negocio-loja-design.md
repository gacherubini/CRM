...
**Referências:** `docs/referencia-viva/README-COMERCIAL.md` (visão), `docs/nao-plano/mercado/README.md` (fosso e ICP),
`docs/nao-plano/vendas/script-venda-outbound.md` (a pergunta que abre a venda),
`portal-gestao/app/loja/sales_overview.py` (read model reusado),
`docs/referencia-viva/specs/2026-08-07-control-overview-loja-agente-design.md` (padrão de página +
degradação).
...
R$ 700–1.200/mês (`docs/nao-plano/mercado/README.md`), o custo de LLM fica em ~0,1% da receita por loja.
...
Boom também respondem. Segundo `docs/nao-plano/mercado/README.md`, o **único** ponto onde nenhum
concorrente chega é amarrar anúncio → conversa → venda → ROI. E `docs/nao-plano/vendas/script-venda-outbound.md`
...
- Vendas são contadas por **`criada_em`**, não `confirmada_em` (`financeiro_calc.py:144`,
  `sales_overview.py:425`) — divergência já documentada em `docs/referencia-viva/handoff-contexto.md:266-267` —
...
- `docs/referencia-viva/README-COMERCIAL.md` diz que IA não aparece como área principal separada — **superado**
...
> v3) dizem *o que o produto faz*. Os três planos em `docs/referencia-viva/planos/2026-08-11-copiloto-fase*`
...
2. **Trocar `repostar_veiculo` por `atribuir_lead`/`cobrar_followup`.** As duas ações da v1 são de
   estoque, mas a pesquisa de mercado (`docs/nao-plano/mercado/README.md`) aponta resposta e follow-up como
