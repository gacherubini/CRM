...
|---|---|---|
| [Loja — quatro telas reprovadas](2026-09-11-loja-ui-quatro-telas.md) | Loja | UI das telas Veículos (foto grande, linha clicável, foto 404 a diagnosticar), Despesas fixas (vazio sem ícone, fonte de marca), Resultado e Resultado financeiro (o `max-width: 64rem` de `9eb58f2` corta os painéis nas duas telas) e Editar venda (redesenho; UUID cru na tela). Só CSS e template; sem rota nem migration. |
| [Motor RPA multiloja em cloud](2026-09-10-motor-rpa-multiloja-cloud.md) | Motor | Arquitetura de execução cloud que isole capacidade, IP e credenciais por loja. O pool atual tem teto global de dois browsers por risco de captcha/WAF e não atende várias lojas simultâneas. VPS por loja é hipótese a comparar, não decisão. |
| [Mover canal Cloud de loja](2026-09-07-mover-canal-de-loja.md) | Chatbot | Script administrativo que troca `whatsapp_canais.loja_id` — a única porta para o chip testado na loja `teste` virar para a loja real, já que o popup recusa número cadastrado e não existe rota que solte o `loja_id`. Leva junto a limpeza do tráfego de teste, sem a qual a conversa velha (única por `(canal_id, telefone)`, sem conferir `loja_id`) sequestra o telefone do lojista depois do corte. **Ler o spec junto.** |
