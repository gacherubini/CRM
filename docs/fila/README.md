...
|---|---|---|
| [Skill `revy-research`](2026-08-23-skill-revy-research.md) | Ferramental (todos) | O agente não tem como achar código: 722 `.py` de projeto contra 10.286 com os cinco `.venv`, e `main.py` de 2.609 linhas. O card constrói um mapa gerado por AST (`arquivo:linha` de rota, modelo, worker, migration, flag, template) com `--verificar` que prova que o mapa não mente, mais `learnings/` e `decisoes/` versionados. A skill é **porta, não caminho**: dá o contexto e roteia para as skills do superpowers. **Spec junto.** Task 1 mexe no `.gitignore` e é pré-requisito das outras. |
