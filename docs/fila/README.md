...
| [Foto de veículo no Portal](2026-08-12-foto-veiculo-upload-portal.md) | Loja / Estoque | Upload por arquivo no form legado. `estoque-api` já tem `POST /v1/veiculos/{id}/fotos/upload`. Não há `adicionar_foto` no `EstoqueClient` nem `<input type="file">` no form. |
| [Central de notificação + simulação pronta](2026-08-12-notificacao-central-simulacao-pronta.md) | Loja / Chatbot | Sino geral **desacoplado** da flag do Copiloto + tipo `simulacao_pronta`. O sino atual (`/app/loja/copiloto/notificacoes.json`) some com o Copiloto off. |
...
| Alerta no grupo de estoque | **Código pronto** (outbox + retry + dead-letter). Residual = smoke e Active ON. |
| WhatsApp dois modos | Spec em [`../referencia-viva/specs/2026-08-12-whatsapp-dois-modos-design.md`](../referencia-viva/specs/2026-08-12-whatsapp-dois-modos-design.md). Plano de implementação ainda não escrito. Coexistência por vendedor foi **descartada**. |
