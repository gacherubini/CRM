...
- **`motor2037` NÃO migra.** Permanece em `gru`. Razão em `docs/plans/2026-07-16-fly-rpa-captcha-opcoes.md`: o RPA já falha com `captcha_login` por reputação de IP de datacenter; IP americano logando em portal de lojista brasileiro piora o scoring e arrisca geo-block. Economia de mover: ~zero (as 5 machines ficam `stopped`).
...
- Modify: `docs/handoff-contexto.md` (arquitetura do lab)

- [ ] **Step 1: Aplicar as edições de toml**

- [ ] **Step 2: Documentar a topologia dividida no README**

Tabela nova: `app2037`/`n8n2037`/`evolution2037`/`suite-pg` em `iad`; `motor2037` em `gru` com a justificativa de IP bancário e o link para `docs/plans/2026-07-16-fly-rpa-captcha-opcoes.md`. Registrar R1 (motor↔pg cross-region) como característica conhecida.
...
```bash
git add deploy/fly/3vm/ docs/handoff-contexto.md
...
- [ ] **Step 5: Atualizar `docs/handoff-contexto.md`** com a data da migração e a pendência de re-pareamento do WhatsApp.
