...
**3. `GET /v1/config/catalogo-bot` e cega para loja, e `instance` nao conserta.**
Ela **nao le `ctx.loja_id`**. Quem responde e `InventoryWriteClient.obter_loja()`
(`app/inventory.py:461`), que bate em `/v1/loja` do Estoque com **um bearer
global, sem slug** — enquanto `provider.buscar` (`inventory.py:205`) usa
`/public/v1/lojas/{slug}/veiculos` e por isso *e* multi-loja. Com N lojas no
Modo 2, todas recebem o catalogo de uma so. E buraco do **contrato com o
Estoque**, nao da credencial: aceitar `instance` ali daria a impressao de estar
resolvido sem mudar nada.

**Como aplicar:** antes de dar `instance` a uma rota, pergunte de onde ela tira
a loja hoje. Se a resposta nao for `ctx.loja_id`, `instance` e teatro.
